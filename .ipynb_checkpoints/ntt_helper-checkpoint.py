# La classe `NTTHelper` a été définie pour permettre à la classe 
# `Polynomial` de bénéficier d'une assistance pour la NTT avec n=256
# spécifiquement pour Dilithium. Ce code est fonctionnel, mais il 
# n'est pas facilement généralisable.

# TODOs:
# - Créer une structure permettant de généraliser ce code au-delà de n=256.
...
class NTTHelper():
    ...
    def br(i, k):
        """
        Inversion des bits d'un entier non signé de k bits
        """
    ...
    def ntt_mul(self, a, b):
        """
        Multiplication puis réduction de Montgomery
    ...
    def to_ntt(self, poly):
        """
        Convertit un polynôme en forme de transformation 
        numérique-théorique (NTT) sur place.
        L'entrée est en ordre standard, la sortie est en ordre 
        inverse de bits.
        Les zetas NTT incluent également le facteur de Montgomery 2^16,
        donc NTT se mappe également au domaine de Montgomery.
        
        Actuellement, seulement implémenté pour n = 256
        """
        if poly.is_ntt:
            raise ValueError("Impossible de convertir un polynôme en forme NTT en forme NTT")
    ...
    def from_ntt(self, poly):
        """
        Convertit un polynôme de la forme de transformation
        numérique-théorique (NTT) en place et multiplie par le 
        facteur de Montgomery 2^16.
        L'entrée est en ordre inverse de bits, la sortie est en ordre standard.
        
        En raison de la multiplication de Montgomery, nous avons:
            f != f.to_ntt().from_ntt()
            f = (1/2^16) * f.to_ntt().from_ntt()
        
        Pour récupérer f, nous faisons:
            f == f.to_ntt().from_ntt().from_montgomery()
            
        Actuellement, seulement implémenté pour n = 256
        """
        if not poly.is_ntt:
            raise ValueError("Impossible de convertir un polynôme qui n'est pas en forme NTT")
